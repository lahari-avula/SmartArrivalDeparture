...
public class MySqlConnectionApp implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		int result = jdbcTemplate.queryForObject("select count(*) FROM smartarrival.customer_info", Integer.class);
		if (result > 0) {
	...
	public Customer getCustomerInfo(String accountId) {
		...
		try {
			customer = jdbcTemplate.queryForObject(query, new Object[] { accountId }, new CustomerMapper());
		} catch (Exception e) {
